...
Promise.all([
  (async () => {
    // Load the full base data (ALL countries + bullets, cities, etc.)
    const base = await fetch('data/countries.json').then(r => r.json());

    // Try to load the optional overlay (e.g., CN videos/universities)
    let overlay = {};
    try {
    ...
    // Merge: per-country keys in overlay augment/override base
    const merged = { ...base };
    for (const [code, extra] of Object.entries(overlay)) {
      merged[code] = { ...(base[code] || {}), ...extra };
    }
  ...
  fetch('data/world.geo.json').then(r => r.json())
    ...
    function renderCountryCard(code) {
        ...
        if (!c) {
            panelContent.innerHTML = `<div class="card"><h2>No 
...
${p.direct}<br/><b>Indirect:</b> ${p.indirect}</li>`).join('')}</ul>
 `: ``}
 <!-- ✅ Video (prefer first item in c.videos; fallback to c.media) -->
${
  (c.videos && c.videos.length)
    ? `
      <h3>${c.videos[0].title || 'Watch video'}</h3>
      <div class="video-wrapper">
        <iframe
          src="${c.videos[0].url}"
          title="${c.videos[0].title || 'video'}"
...
${
  (c.universities && c.universities.length)
    ? `
      ...
      <ul>
        ${c.universities.map(u => `<li><a href="${u.url}" target="_blank">${u.name}</a></li>`).join('')}
      </ul>
    `
    : (c.university ? `<p><a href="${c.university}" target="_blank">Visit Friendly University</a></p>` : '')
}
 <div id="cityPins"></div>
 ...
 `;
    ...
    const layer = L.geoJSON(WORLD, {
        style: {
            color: '#888', weight: 0.8, fillColor: '#dfe8ff',
            fillOpacity: 0.6
        },
